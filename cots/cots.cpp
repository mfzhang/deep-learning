...
Cots::Cots(int input_size, int input_channels, int filter_size, int filter_channels, int batch_size, int block_size, \
        int step, int process_num, int thread_num, int pooling_size, float learning_rate, float learning_rate_alpha, float alpha, float momentum, float lambda, string in_address, string out_address)
{
    init(input_size, input_channels, filter_size, filter_channels, batch_size, block_size, step, process_num, \
            thread_num, pooling_size, learning_rate, alpha, learning_rate_alpha, momentum, lambda, in_address, out_address);
}
        ...
        int step, int process_num, int thread_num, int pooling_size, float learning_rate, float learning_rate_alpha, float alpha, float momentum, float lambda, string in_address, string out_address)
{
    ...
    this->_input_address = in_address;
    this->_output_address = out_address;
    this->_lambda = lambda;
    assignMemory();
    ...
    for(int epoch_idx = 0; epoch_idx < epoch; epoch_idx++)
    {
        for(int batch_idx = 0; batch_idx < batch_all_size/this->_pars->batch_size; batch_idx++)
        {
            ...
            if(me == 0)
            {
                ...
                cout << "batch_idx is " << batch_idx << "\n";
                //预处理图片
                preprocess(this->_input_address, batch_idx, this->_pars->batch_size, this->_pars->input_channels, this->_pars->input_size, this->_pars->input, type);
            }
           ...
           /*        if(me ==0 )
                      {
                            cout << "==========input and reconstruct=============\n";
                      for(int i = 0; i< 100; i++)
                      {
                      cout << this->_pars->input[i] << ":"<< this->_pars->receive_reconstruct[i]<< "\t";
                      }
                      cout << endl;
                        cout << "==========pooling and lcn=============\n";
                      for(int i = 0; i< 100; i++)
                      {
                          cout<< this->_pars->receive_pooling[i] << ":" << this->_pars->receive_lcn[i]<< "\t";
                      }
                      cout << endl;
                      }*/
            if((me == 0) && (epoch_idx == epoch -1))
...
void Cots::preprocess(string in_address, int batch_idx, int num, int channels, int size, float *input, bool type)
{
    ifstream fin(in_address.c_str(), ios::binary);
    if(type)
    {
...
void Cots::normalizeWeight()
{
    ...
    int length1 = this->_pars->process_num*this->_pars->filter_channels*this->_pars->block_size*this->_pars->block_size*this->_pars->input_channels;
...
{
    int length1 = this->_pars->filter_channels*this->_pars->block_size*this->_pars->block_size*this->_pars->input_channels;
    for(int m = 0; m < length1; m++)
    {
        int length2 = this->_pars->filter_size*this->_pars->filter_size;
        float sum = 0;
        ...
        for(int i = 0; i < length2; i++)
        {
            graident[i + m*length2] = graident[i + m*length2]/sqrt(sum) - project_weight[i + m*length2]*sum1/sqrt(sum);
        }
    }
}
                        ...
                        for(int t = -1; t < this->_pars->pooling_size - 1; t++)
                        {
                                    ...
                                    &&(row_block + m + k >= 0)&&(row_block + m + k < this->_pars->out_size/this->_pars->block_size))
                            {
                                ...
                                switch(type)
                                {
                                    ...
                                    case 1:
                                        {
                                            if(this->_pars->receive_pooling[start_in_receive + m*this->_pars->out_size + n] != 0)
...
void Cots::computeLcn(int me, int process_idx, bool type)
{
    ...
    for(int i = 0; i < this->_pars->batch_size; i++)
    {
        float *sum1 = new float[this->_pars->pooling_size*this->_pars->pooling_size];
        float *sum2 = new float[this->_pars->pooling_size*this->_pars->pooling_size];       
        for(int j = 0; j < this->_pars->filter_channels; j++)
        {
            //block的起点
            int id = process_idx*this->_pars->thread_num + me;
                ...
                {                    
                    ...
                    for(int k = -1; k < this->_pars->pooling_size - 1; k++)
                    {
                        for(int t = -1; t < this->_pars->pooling_size - 1; t++)
                        {
                            ...
                            {
                                if(type)
                                    sum1[m*this->_pars->pooling_size + n] += gaussion[(k+1)*this->_pars->pooling_size + \
                                                                             t + 1]*this->_pars->receive_pooling[start + (k+m)*this->_pars->out_size + t+n];
                                else
                                {
                                    sum2[m*this->_pars->pooling_size + n] += gaussion[(k+1)*this->_pars->pooling_size + t \
                                                                             + 1]*this->_pars->receive_lcn[start + (k+m)*this->_pars->out_size + t+n] \
                                                                             *this->_pars->receive_lcn[start + (m+k)*this->_pars->out_size + t+n];
                                }
                            }
                        }
                    }
                ...
                {       	
                    ...
                    if(type)
                    {
                        this->_pars->block_lcn[pos_in_block] = this->_pars->receive_pooling[start + m*this->_pars->out_size + n] \
                                                               - sum1[m*this->_pars->pooling_size + n];
                            cout << this->_pars->block_lcn[pos_in_block] << ":" << this->_pars->receive_pooling[start + m*this->_pars->out_size + n] << "\t";
                        
                    }
                    else
                    {
                        this->_pars->block_lcn[pos_in_block] = this->_pars->receive_lcn[start + m*this->_pars->out_size + n] \
                                                               /(sqrt(sum2[m*this->_pars->pooling_size + n]) > 0.01 ? sqrt(sum2[m*this->_pars->pooling_size + n]) : 0.01);

                    
     //                   if(me == 0&& process_idx == 5&&!type)
...
void Cots::updateW(int me, int batch_idx)
{
    ...
    {       	
        ...
        buildH(me, this->_pars->block_pooling, this->_pars->receive_pooling, process_idx, false);
    ...
    MPI_Allreduce(&send_delta_alpha, &this->_delta_alpha, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
             /*   if(me == 0)
                  {
                      cout << "=========weight and delta_weight=========\n";
                    for(int i = 0; i < 100; i++)
                    {
                        cout << block_origin_weight[i]  << ":" << block_dw2[i] << "\t";
                    }
                    cout << endl;
                  }*/
    delete[] block_dw1;
