...
int main()
{
	ifstream fin("../../../crd/deeplearning/data/stl10/unlabeled_X.bin", ios::binary);
	ofstream fout("./data/preocessed_unlabed.bin", ios::binary);
	int num = 5000;
	int channels = 3;
	int size = 96;
...
//					cout << value[i*channels*size*size + j*size*size + m*size + n] << endl;
				}
			}
